...
function serviceWorker(options = {}) {
  ...
  return {
    ...
    async writeBundle() {
      ...
      console.log(
        `${colorette.blueBright(colorette.bold('Service Worker data:'))} ${colorette.green(
          `${colorette.bold(finalPaths.length)} fetch paths, ${colorette.bold(
            relativePaths.length
          )} static files for`
        )} ${colorette.yellow(colorette.bold(formatBytesHuman(sum)))}`
      );
    },
  };
}
