...
const { blueBright, bold, green, yellow, magenta } = require('colorette');
...
function generateSWDataJS(date, paths, runtimeTypes) {
  let pathBuffer = '';
...
module.exports = function serviceWorker(options = {}) {
  let {
    dir = './public',
    swName = 'sw.js',
    dev = false,
    manualPaths = [],
    prefetchExtensions = ['js', 'css', 'html', 'svg'],
    runtimeTypes = [/^font\//],
    verbose = true,
  ...
  return {
    ...
    async writeBundle() {
      ...
      if (dev) {
        ...
        fs.writeFileSync(swPath, generateDevSWDataJS());
        if (verbose) {
          console.log(
      ...
      for (const path of paths) {
        ...
        // Check match extension from config
        if (matchExtension(prefetchExtensions, extension)) {
